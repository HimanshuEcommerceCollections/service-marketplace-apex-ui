/* eslint-disable */
// Apex cleaning service-page runtime.
//
// Ported (near-verbatim) from the four <script> blocks in
// apex-service-cleaning-preview_extracted.html:
//   1. shared behaviour (reveal-on-scroll, count-up) + the configurator engine
//   2. the testimonial carousel
//   3. the .anav navbar + footer reveal + watermark parallax
...
//      is replaced by an exported mountCleaning(slug) that the page's useEffect
//      calls once the DOM is committed.
...
// only the `cleaning` spec is activated on this page via the slug argument.

import { testimonials as TDATA } from '../../data/cleaning/testimonials';

export function mountCleaning(slug) {
  const cleanups = [];
